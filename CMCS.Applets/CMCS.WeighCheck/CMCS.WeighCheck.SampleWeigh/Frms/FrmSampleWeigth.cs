        ...
        private void txtInputSampleCode_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (this.CurrentFlowFlag == eFlowFlag.等待扫码)
                {
                    ...
                    {
                        // 查找3天内未绑定采样单的样桶记录
                        CmcsRCSampleBarrel rCSampleBarrel = commonDAO.SelfDber.Entity<CmcsRCSampleBarrel>("where BarrelCode=:BarrelCode and CreateDate>=:CreateDate and IsDeleted=0 order by CreateDate desc", new { BarrelCode = barrelCode, CreateDate = DateTime.Now.Date.AddDays(-3) });
