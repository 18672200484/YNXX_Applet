...
namespace CMCS.CarTransport.DAO
{
	...
	public class CarTransportDAO
	{
		...
		{
			DateTime dt = buyFuelTransport.InFactoryTime.AddHours(-commonDAO.GetCommonAppletConfigInt32("汽车分批时间点"));
			return SelfDber.Entity<CmcsInFactoryBatch>("where Batch like '%-'|| to_char(:CreateDate,'YYYYMMDD') ||'-%' and MineId=:MineId  and BatchCreateType=1 and IsDeleted=0", new { CreateDate = dt, MineId = buyFuelTransport.MineId });
		}
		...
		public List<View_UnFinishTransport> GetUnFinishTransportByCarNumber(string carNumber, string sqlWhere)
		{
			...
			if (!string.IsNullOrEmpty(carNumber))
			{
				return res.Where(a =>
				{
					if (!string.IsNullOrEmpty(a.CarNumber) && a.CarNumber.ToUpper().Contains(carNumber.ToUpper())) return true;
