    ...
    public class YB19DynamicAreaLeder
    {
        ...
        public static bool Connect(string ip)
        {
            unsafe
            {
                ...
                nResult = BxScreenClient_connect(screen, ip, 5005, true);
                return nResult == 0;
            }
        }

        //测试图文区 发送文本 测试颜色 测试字型
        public static bool SendTextColor(string ip, string value1, string value2 = "")
        {
            string Content = value1 + (!string.IsNullOrEmpty(value2) ? Environment.NewLine + value2 : "");
            unsafe
            {
                uint screen;
                uint program;
                ...
                nResult = Create_FColor(&black, 0, 0, 0);
                nResult = Create_FFont(&font, "宋体", 14, false);
                ...
                nResult = Create_TextCaptionBxArea(&area, 0, 0, 96, 32, profile);

                nResult = Create_TextBxPage4(&page, Content, font, red, black);
                nResult = TextBxPage_setHorizontalAlignment(page, 0);
